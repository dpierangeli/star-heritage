...
if(isset($_REQUEST['id_gilda'])===FALSE){  
  ...
  $query= "SELECT gilda.nome, gilda.id_gilda, gilda.tipo, gilda.immagine, gilda.Anonimo, codtipogilda.descrizione FROM gilda JOIN codtipogilda ON gilda.tipo = codtipogilda.cod_tipo WHERE gilda.visibile = 1 ORDER BY gilda.tipo, gilda.nome";
...
<?php /*Visualizzazione estesa gilda*/
    ...
    $query_gilda= "SELECT gilda.nome, gilda.id_gilda, gilda.tipo, gilda.immagine, gilda.Anonimo, codtipogilda.descrizione FROM gilda JOIN codtipogilda ON gilda.tipo = codtipogilda.cod_tipo WHERE gilda.id_gilda=".gdrcd_filter('num',$_REQUEST['id_gilda']);
    $result_gilda=gdrcd_query($query_gilda, 'result');
    $rowGuild=gdrcd_query($result_gilda, 'fetch');
    if ($rowGuild['Anonimo']!=1 || $_SESSION['permessi']>=GAMEMASTER)
    {
