...
<?php /*Inserimento di un nuovo record*/
    if ($_POST['op']=='insert')
    {
	   ...
	   gdrcd_query("INSERT INTO abilita (nome, descrizione, car, id_razza, val_init, tipo_forza) VALUES ('".gdrcd_filter('in',$_POST['nome'])."', '".gdrcd_filter('in',$_POST['descrizione'])."', ".gdrcd_filter('num',$_POST['car']).", ".gdrcd_filter('num',$_POST['id_razza']).", '".gdrcd_filter('in',$_POST['val_init'])."', ".gdrcd_filter('num',$_POST['tipo_forza']).")"); 
	   ?>       
	   <div class="warning">
...
<?php /*Modifica di un record*/
	if (gdrcd_filter('get',$_POST['op'])=='modify'){ 
       ...
       gdrcd_query("UPDATE abilita SET nome ='".gdrcd_filter('in',$_POST['nome'])."', descrizione ='".gdrcd_filter('in',$_POST['descrizione'])."', car = ".gdrcd_filter('num',$_POST['car']).", id_razza = ".gdrcd_filter('num',$_POST['id_razza']).", val_init = '".gdrcd_filter('in',$_POST['val_init'])."', tipo_forza = ".gdrcd_filter('num',$_POST['tipo_forza'])." WHERE id_abilita = ".gdrcd_filter('num',$_POST['id_record'])." LIMIT 1");?>
   	   <div class="warning">
...
<?php /*Form di inserimento/modifica*/
    ...
    <form action="main.php?page=gestione_abilita"
		  ...
		  <div class='form_field'>
	         <select name='car'>
				...
				<option value="6" <?php if($loaded_record['car']==6){echo 'SELECTED';} ?>>
				  <?php echo gdrcd_filter('out',$PARAMETERS['names']['stats']['car6']); ?></option>
			 </select>
		  </div>
      ...
      <div class='form_field'>
        <input name="val_init" value="<?php echo $loaded_record['val_init']; ?>" />
      </div>
      
	    <div class='form_label'>
      Utilizzo poteri forza:
      </div>
      <div class='form_field'>
        <select name='tipo_forza'>
          <option value="0" >Non usare PF </option>
          <option value="1" SELECTED >Usa PF per potenziare </option>
          <option value="2" >Usa PF come potere</option>
          <option value="3" >Recupera PF </option>
        </select>
      </div>
...
<?php if (isset($_REQUEST['op'])===FALSE) { /*Elenco record (Visualizzaione di base della pagina)*/
	...
	*/
	...
	//Lettura record
	$result 	= gdrcd_query("SELECT id_abilita, nome, car, val_init, tipo_forza FROM abilita ORDER BY nome LIMIT ".$pagebegin.", ".$pageend."", 'result'); 
       ...
       <table>
          ...
          <tr>
		     ...
		     <td class="casella_titolo"><div class="titoli_elenco"><?php echo gdrcd_filter('out',$MESSAGE['interface']['administration']['skills']['car']); ?></div></td>
			   <td class="casella_titolo"><div class="titoli_elenco"><?php echo 'Valore Iniziale'; ?></div></td>
			   <td class="casella_titolo"><div class="titoli_elenco"><?php echo 'Uso Punti Forza'; ?></div></td>
	      ...
	      <tr class="risultati_elenco_record_gestione">
		     ...
		     <td class="casella_elemento">
			   <div class="elementi_elenco">
			    ...
			    $vall = 'non usare';
			    //
			    if (gdrcd_filter('num',$row['tipo_forza']) == 1)
			      $vall = 'potenzia';
          if (gdrcd_filter('num',$row['tipo_forza']) == 2)
			      $vall = 'potere';
			    if (gdrcd_filter('num',$row['tipo_forza']) == 3)
			      $vall = 'recupera';
			    //		    
			    echo $vall; 
			    ?>
