...
<?php /*Inserimento di un nuovo record*/
    if ($_POST['op']=='insert') 
    {
	   ...
	   $query="INSERT INTO news (data, titolo, testo, tipo) VALUES (NOW(), '".gdrcd_filter('in',$_POST['titolo'])."', '".gdrcd_filter('in',$_POST['testo'])."', ".gdrcd_filter('num',$_POST['tipo']).")";
...
<?php /*Modifica di un record*/
	if (gdrcd_filter('get',$_POST['op'])=='doedit'){
	   ...
	   gdrcd_query("UPDATE news SET titolo ='".gdrcd_filter('in',$_POST['titolo'])."', testo ='".gdrcd_filter('in',$_POST['testo'])."', tipo=".gdrcd_filter('in',$_POST['tipo'])." WHERE id_news = ".gdrcd_filter('num',$_POST['id_record'])." LIMIT 1");
?>
   	   <div class="warning">
...
<?php /*Form di inserimento/modifica*/
    ...
    <form action="main.php?page=gestione_news"
      ...
      <div class='form_field'>
        <select name="tipo">
          <option value="0" <?php echo ($loaded_record['tipo']==0 ? 'selected': ''); ?> >ON</option>
          <option value="1" <?php echo ($loaded_record['tipo']==1 ? 'selected': ''); ?>>OFF</option>
        </select>
      </div>
		  <div class='form_label'>
...
{ /*Elenco record (Visualizzaione di base della pagina)*/
  ...
  $pagebegin=(int)gdrcd_filter('get',$_REQUEST['offset'])*$PARAMETERS['settings']['records_per_page'];
	...
	$record_globale=gdrcd_query("SELECT COUNT(*) FROM news");
