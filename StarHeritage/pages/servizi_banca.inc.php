...
//
// Bene, abbiamo calcolato lo stipendo di un giorno, proviamo però ad addebitare lo stipendio
$stipendioGior = $stipendio;
$oggi = new DateTime();
if ($ultimo == null)
  $last_stip = new DateTime();
else
  $last_stip = new DateTime($ultimo);
$interval = $oggi->diff($last_stip);
$days = $interval->days;
if ($days>5)
  $days = 5;
if ($days>1 && $days<=5)
  $stipendio = $stipendio*$days;
?>
...
if((isset($_POST['op'])===TRUE)&&($_POST['op']=='bonifico')){
		...
		} else if (gdrcd_filter('in',$_POST['causale']) == "Causale") {
      echo '<div class="warning">Causale non valida</div>';
    }
      ...
      // No, non trasferiamo qui i soldi ma li memorizziamo nella tabella dei bonifici da confermare
      //gdrcd_query("UPDATE personaggio SET banca = banca + ".gdrcd_filter('num',$_POST['ammontare'])." WHERE nome = '".$_POST['beneficiario']."' LIMIT 1");
gdrcd_query("INSERT into bonifici (bonificante, destinatario, quantita, causale, data_bonifico) VALUES('".$_SESSION['login']."', '".gdrcd_filter('in',$_POST['beneficiario'])."', ".gdrcd_filter('num',$_POST['ammontare']).", '".gdrcd_filter('in',$_POST['causale'])."', NOW())");
			...
			// Niente messaggio, lo mandiamo quando il bonifico verra' confermato
      ...
      echo "<div class='warning'>Bonifico preso in carico, il denaro verrà trasferito quando la Banca avrà accettato la transazione. Nel caso non ricevessi notizie a breve contatta uno degli amministratori.</div>";
         } ?>
...
<?php /*Visualizzazione di base*/
	...
	<div class="status_bancario">
       ...
       <!-- Stipendio -->
	   <?php echo gdrcd_filter('out',$MESSAGE['interface']['bank']['per_day'].": ".$stipendioGior." ".$PARAMETERS['names']['currency']['plur']);
     if ($stipendio > $stipendioGior)
      echo "(stipendio arretrato: ".gdrcd_filter('out', $stipendio).")";
     ?><br />
	   <!-- In tasca -->
