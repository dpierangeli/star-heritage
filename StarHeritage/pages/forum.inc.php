...
if($_POST['op']=='edit')
{
	...
	if ($row['autore'] == $_SESSION['login'] || ($row['autore'] != $_SESSION['login'] && $_SESSION['permessi'] >= MODERATOR))
	{
		...
		$time=strftime('%d').'/'.strftime('%m').'/'.strftime('%Y').' '.(strftime('%H')+$PARAMETERS['date']['fusopar']).':'.strftime('%M');
...
<form action="main.php?page=forum"
      method="post">
  <font color="#cecab8"><div class="form_label">
    Risposta rapida
  </div>
  <div class="form_field">
...
$result = gdrcd_query("SELECT id_araldo, nome, tipo, proprietari, sequence, sequence_int FROM araldo ORDER BY sequence DESC, tipo, sequence_int, nome", 'result');
...
{ 
  ...
  <tr><!-- Intestazione tabella -->
    <td colspan="2"><div class="capitolo_elenco">
      <?php echo gdrcd_filter('out',$MESSAGE['interface']['forums']['type'][$ultimotipo]); ?>
    </div></td>
  </tr> 
  <?php 
  } //if
  ...
  $post_tot = gdrcd_query("SELECT COUNT(MA.id_messaggio) AS num FROM messaggioaraldo AS MA LEFT JOIN araldo_letto AS AL ON MA.id_messaggio=AL.thread_id AND AL.nome='".$_SESSION['login']."' WHERE MA.id_messaggio_padre = -1 AND MA.id_araldo = ".$row['id_araldo']); 
  $post_read = gdrcd_query("SELECT COUNT(MA.id_messaggio) AS num FROM messaggioaraldo AS MA JOIN araldo_letto AS AL ON MA.id_messaggio=AL.thread_id AND AL.nome='".$_SESSION['login']."' WHERE MA.id_messaggio_padre = -1 AND MA.id_araldo = ".$row['id_araldo']); 
  
  $news_obj = $new_msg['num'] + ($post_tot['num'] - $post_read['num']);
  ?>
    ...
    <td class="forum_main_post_author">
	   <div class="forum_date_big">
	      <?php 
            ...
            { 
              // Vedo il numero di post nuovi solo per i Forum a cui ho accesso..
              if ($news_obj == 1)
              {
              ...
              {
                  echo $news_obj. ' ' . $MESSAGE['interface']['forums']['topic']['new_posts']['plur'];
              }
            }
					}
			?>
...
/*Carico l'elenco dei forum*/
$result = gdrcd_query("SELECT MA.id_messaggio, MA.titolo, MA.autore, MA.data_messaggio, MA.importante, MA.chiuso, AL.id AS new_msg, if(ifnull(AL.id,0)=0,1,0) AS msg_seq, ifnull((SELECT MAX(data_messaggio) FROM messaggioaraldo ms WHERE ms.id_messaggio_padre=MA.id_messaggio), MA.data_messaggio) AS latest FROM messaggioaraldo AS MA LEFT JOIN araldo_letto AS AL ON MA.id_messaggio=AL.thread_id AND AL.nome='".$_SESSION['login']."' WHERE MA.id_messaggio_padre = -1 AND MA.id_araldo = ".gdrcd_filter('num',$_REQUEST['what'])." ORDER BY MA.importante DESC, msg_seq DESC, latest DESC LIMIT ".$pagebegin.", ".$PARAMETERS['settings']['posts_per_page']."", 'result'); 
