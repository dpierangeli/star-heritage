...
} elseif (strtotime($record['ora_entrata']) > strtotime($record['ora_uscita'])||(strtotime($record['ultimo_refresh'])+300) > time())
{
	...
	/*Se la postazione è stata esclusa*/  
    $errorMsg =  '<div class="error_box"><h2 class="error_major">'.$MESSAGE['warning']['double_connection'].'</h2></div>';
	/*Registro l'evento (Tentativo di connessione da postazione esclusa)*/
    gdrcd_query("INSERT INTO log (nome_interessato, autore, data_evento, codice_evento ,descrizione_evento) VALUES ('".$login1."', 'Login_procedure', NOW(), ".BLOCKED.", '".$_SERVER['REMOTE_ADDR']."')");
    exit();
    ...
    if (empty($record)===FALSE) {
      $errorMsg = '<div class="error_box"><h2 class="error_major">'.'Utente non trovato: '.'</h2></div>';
      $errorMsg += '<div class="error_box"><h2 class="error_major">'.'Nome inserito: '.gdrcd_filter('in',$login1).'</h2></div>';
      $errorMsg += '<div class="error_box"><h2 class="error_major">'.'Pass inserita: '.gdrcd_filter('in',$pass1).'</h2></div>';
    }
...
<a href="mailto:<?php echo $PARAMETERS['info']['webmaster_email']?>">
  <?php echo $PARAMETERS['info']['webmaster_email']?>
</a>.
</div>
