...
<form action = "<?php echo $_SERVER['SCRIPT_NAME'] . '?' . $_SERVER['QUERY_STRING']; ?>" method="post">
   ...
   <div class="form_field" >
	  ...
	  <select id="car6" name="car6" style="width:70px;" onchange="sumPoints();">
		<?php for($i=1; $i<=$PARAMETERS['settings']['initial_cars_cap']; $i++){ ?>
		   <option value="<?php echo $i; ?>" <?php if(gdrcd_filter('num',$_POST['car6'])==$i){ echo 'SELECTED'; } ?> >
		      <?php echo $i; ?>
		   </option>
		<?php } ?> 
	  </select>
...
//
gdrcd_query("INSERT INTO personaggio (nome, cognome, pass, data_iscrizione, email, sesso, id_razza, car0, car1, car2, car3, car4, car5, salute, salute_max, soldi, esperienza, Taglia, eta, Sens, forza, forza_max $lastpasschange_field) VALUES ('".gdrcd_capital_letter(gdrcd_filter('in',$_POST['nome']))."', '".gdrcd_filter('in',$_POST['cognome'])."', '".gdrcd_encript($pass)."', NOW(), '".gdrcd_filter('in',$_POST['email'])."', '".gdrcd_filter('in',$_POST['genere'])."', ".gdrcd_filter('num',$_POST['razza']).", ".gdrcd_filter('num',$_POST['car0']).", ".gdrcd_filter('num',$_POST['car1']).", ".gdrcd_filter('num',$_POST['car2']).", ".gdrcd_filter('num',$_POST['car3']).", ".gdrcd_filter('num',$_POST['car4']).", ".gdrcd_filter('num',$_POST['car5']).", ".	gdrcd_filter('num',$PARAMETERS['settings']['max_hp']).", ".gdrcd_filter('num',$PARAMETERS['settings']['max_hp']).", ".gdrcd_filter('num',$PARAMETERS['settings']['first_money']).", ".gdrcd_filter('num',$punti_iniz).", ".$TagliaRaz.", ".$etaRaz.", ".gdrcd_filter('num',$_POST['car6']).", ".gdrcd_filter('num',$_POST['car6']).", ".gdrcd_filter('num',$_POST['car6'])." $lastpasschange_value)");
//
