  ...
  switch (strtolower(trim($mode))) {
    case 'query':
      ...
      switch (strtoupper(substr(trim($sql), 0, 6))) {
        ...
        default:
          //mysqli_query($db_link, "SET time_zone='".$PARAMETERS['date']['fuso']."';");
...
function gdrcd_check_tables($table) {
  ...
  while ($field = gdrcd_query($describe, 'object')) {
    ...
    switch ($field->Key) {
      case 'PRI': $field->definition .= ' PRIMARY KEY';
        break;
      case 'UNI': $field->definition .= ' UNIQUE KEY';
        break;
      case 'MUL': $field->definition .= ' KEY';
        break;
    }
...
function gdrcd_encript($str) {
  ...
  if ($encript_password == 'ON') {
    switch ($encript_algorithm) {
      case 'MD5': $str = md5($str);
        break;
      case 'SHA-1': $str = sha1($str);
        break;
    }
  }
...
function gdrcd_format_time($time_in) {
  //return date('H:i', strtotime('+2 hours', strtotime($time_in)));
...
function gdrcd_format_datetime($datetime_in) {
  //return date('d/m/Y H:i', strtotime('+2 hours', strtotime($datetime_in)));
...
function gdrcd_chatcolorsave($str) {
  $search = array(
      '#\&lt;(.+?)\&gt;#is',
      '#\[(.+?)\]#is',
  );
...
function gdrcd_rankCost($rank) {
  // 01-20; 21-36; 37-48; 49-56; 57-62; 63-67; 68-100
  //  1       2     3      4      5      6       7
  $cost = 0;
  for ($i = 1; $i <= $rank; $i++) {
    if ($i <= 20)
...
function gdrcd_singleRankCost($rank) {
  // 01-20; 21-36; 37-48; 49-56; 57-62; 63-67; 68-100
  //  1       2     3      4      5      6       7
  if ($rank <= 20)
    return 1;
...
/**
 * Nasconde dalla stringa le parti all'interno di { }, però solo se il PG non è un master o non è il PG che ha scritto
 * @param string $str: la stringa da controllare
 * @return $str con la parti colorate
 */
function gdrcd_hidechat($str) {
  $search = array(
      '#\{(.+?)\}#is'
  );
  $replace = array(
...
/**
 * Evidenzia il nome del PG passato nella stringa di chat
 * @param string $str: la stringa da controllare
 * @return $str con la parti colorate
 */
function gdrcd_highlightpg($str, $pg) {
  $search = array(
      $pg,
      strtolower($pg)
  );
  $replace = array(
      '<span class="color3">' . $pg . '</span>'
  );
